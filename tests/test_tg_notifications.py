...
async def test_available_topics_follow_roles(db_session):
    ...
    async with db_session() as db:
        ...
        assert await _available_topics(db, client) == ["bookings", "reminders", "promos"]
        assert set(await _available_topics(db, master)) == {"bookings", "reminders", "promos", "warehouse", "reviews"}
        assert set(await _available_topics(db, inv_admin)) == {"bookings", "reminders", "promos", "warehouse", "reviews", "reports"}
        assert set(await _available_topics(db, creator)) == {"bookings", "reminders", "promos", "warehouse", "reviews", "reports"}
