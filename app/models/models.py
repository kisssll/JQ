...
class User(Base):
    ...
    # Мессенджер отказал в доставке насовсем (бот заблокирован, чат удалён).
    # Привязку НЕ стираем: человек может разблокировать бота, и тогда всё
    # вернётся само, без повторного подключения. Пока отметка стоит, доставка
    # идёт запасным каналом, а профиль зовёт переподключить. Снимается, когда
...
class ConsentDocument(str, enum.Enum):
    ...
    #: Согласие на рекламную рассылку вечерних окон со скидкой. Отдельным
    #: документом, потому что ч. 1 ст. 18 закона «О рекламе» требует
    #: ПРЕДВАРИТЕЛЬНОГО согласия получателя, а доказывать его обязан
    #: рекламодатель. Настройка в профиле не доказывает ни когда, ни как.
