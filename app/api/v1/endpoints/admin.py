...
async def salon_restore(sid: int, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    salon.is_active = True
    _audit(db, admin.id, "salon_restore", "salon", sid, f"{salon.name}: восстановлен", salon_id=sid)
    await db.commit()
    return _back("salons", ok=f"«{salon.name}» восстановлен")
...
async def salon_delete(sid: int, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    salon.is_active = False
    _audit(db, admin.id, "salon_delete", "salon", sid, f"удалён «{name}»", salon_id=sid)
    await db.commit()
