...
# Токен маркировки рекламы, выдан ОРД. Уходит и в текст под блоком, и в саму
# реферальную ссылку — гайд Т‑Банка требует обоих мест. QR пересобран из
# ссылки с токеном: .venv/bin/python scripts/build_tbank_qr.py
TBANK_ERID = "CQH36pWzJqVKoCSjqr8NnZXokwfAF7rkk9ADarMixdWB37"

# Идентификатор креатива в кабинете ОРД. На страницах не показывается —
# нужен только для отчётности и чтобы связать креатив с токеном, когда
# придёт пора сдавать статистику.
TBANK_ORD_CREATIVE_ID = "c-FXKl-HqBsrFEZjzxgkYKkYnVAkJ"
