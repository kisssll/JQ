...
_TOUR = [
    ("booking-link", "Клиент записывается сам по ссылке",
     ...
     "свободное время — без звонков и переписки.", 727, 583),
    ...
    ("records", "Все записи — в одном списке",
     ...
     "прямо с телефона.", 680, 533),
    ("client-card", "Карточка клиента",
     "Сколько раз приходил, сколько потратил и ваши заметки. «Аллергия на "
     "акриловую пудру» больше не потеряется в переписке.", 688, 885),
    ("overview", "Выручка за неделю",
     "Сколько заработали за неделю и в какие дни больше всего.", 647, 535),
]
