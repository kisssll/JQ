# app/jobs/ask_promo_consent.py
"""Разовая рассылка вопроса о согласии на рекламную рассылку вечерних окон.

Запускается ВРУЧНУЮ и один раз, не по расписанию:

    docker exec rumi-staging-app python -m app.jobs.ask_promo_consent          # показать, кому уйдёт
    docker exec rumi-staging-app python -m app.jobs.ask_promo_consent --send   # отправить
...
Спрашиваем тех, кому рассылка пришла бы по прежней схеме «включено по
умолчанию» (фактически она не уходила никому — ни одна акция не была
включена): у кого есть
...
async def main(send: bool) -> None:
    ...
    for user_id, _ in picked:
        ...
        job = await pool.enqueue_job("ask_promo_consent", user_id)
        if job is not None:
            queued += 1
